...
- Reconstruction: SphereSfM → COLMAP `images/` + `sparse/0` (**gnomonic** `SIMPLE_PINHOLE` 90° cube faces; Votion remaps `sphere_cubic_reprojecer` PNGs — Index #57). Viewport after Reconstruct is an **orbit of the sparse cloud** (`points3D.bin` + camera centers), not cube-face stills. Not WorldFM `transforms.json`.
